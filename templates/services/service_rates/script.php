<script>
function process_service_rate(rate_id,params_tpl)
{
	var rate_data_container = $('.rate-data');
	rate_data_container.empty();
	$.ajaxShort({
		dataType:'HTML',
		data:{
		...
		success:function(data)
		{
			...
			rate_data_container.append(html);
			$.each($('.rate-data').find('form'),function(i,el)
			{
				switch (this.id) {
					...
					case 'rate-weight':
						...
						break;
					case 'rate-weight-volume':
						dataSelect2.call(this,$('.select2me',this));
						new $ajaxForm({
							name:'rate-weight-volume',
							table:'Тарифы2_Вес_Объем',
							table_form:true,
							t_head:['','','Категория груза','ТО кг','Фрахт объем'],
							add:true,
							del:true,
							add_nav:true,
							sub_form:true,
						...
						done_func:function()
						{
							...
							$.ajaxShort({
								data:{
									...
									query:"exec SP_Тарифы_Копирование_тарифа " + rowid + ",'" + name + "'"
								},
								success:function()
								{
									$(grid).trigger("reloadGrid",{current:true});
