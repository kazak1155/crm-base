<style>
#params-content input
{
	min-width:10px;
	max-width:30px;
	text-align: center;
}
#params-content p
{
	line-height: 300%;
	font-size: 16px;
}
#params-content div
{
	margin: 5px 0;
	padding: 0 10px;
	border: 2px dotted #346392;
}
</style>
<div class="rate-tpl-container">
	<fieldset class="slide-set">
		<legend>Параметры тарифа</legend>
		<form id="rate-params">
			<div id="params-content">
				<div>
					<p>
						За каждый дополнительный килограмм в 1м<sup><small>3</small></sup> -
						<input name="overweight_price" value="<?php echo $this->req_data['overweight_price']['value'] ?>"/>
						Евро к ставке.
					</p>
				</div>

				<div>
					<p>
						При весе груза более <input name="self_calc_overweight" value="<?php echo $this->req_data['self_calc_overweight']['value'] ?>"/> - идивидуальный просчет
					</p>
				</div>
				<div>
					<p>
						Минимальная стоимость заказа: <input name="min_price" value="<?php echo $this->req_data['min_price']['value'] ?>"/> Евро
					</p>
				</div>
				<div>
					<p>
						Оформление экспортной =
						<input name="EX1" value="<?php echo $this->req_data['EX1']['value'] ?>"/>
						Евро.
					</p>
				</div>	
				<div>
					<p>
						Складская обработка = Объем * 
						<input name="warehouse_handling" value="<?php echo $this->req_data['warehouse_handling']['value'] ?>"/>
						Евро.
					</p>
				</div>
			</div>
		</form>
	</fieldset>
</div>
